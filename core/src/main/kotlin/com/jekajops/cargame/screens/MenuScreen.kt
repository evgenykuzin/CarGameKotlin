...
import com.jekajops.cargame.actors.scene2d.AudioButton
import com.jekajops.cargame.actors.scene2d.LeaderBoardButton
import com.jekajops.cargame.actors.scene2d.Score
...
class MenuScreen : Screen {
    ...
    constructor() {
        ...
        if (Gdx.app.type == Application.ApplicationType.Android || Gdx.app.type == Application.ApplicationType.iOS) {
            table.add(LeaderBoardButton()).expand().size(GameSettings.WIDTH * 0.15f).center().colspan(3).padBottom(
                GameSettings.WIDTH * 0.05f)
        } else {
            table.add().expand().size(GameSettings.WIDTH * 0.15f).center().colspan(3).padBottom(
                GameSettings.WIDTH * 0.05f)
        }
